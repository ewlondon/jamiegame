...
canvas.width = 1024; // Your canvas width
canvas.height = 768; // Your canvas height
...
const playerHurtSFX = Array(SFX_POOL_SIZE)
    .fill()
    .map(() => new Audio("player_hurt.mp3"));
const enemyHurtSFX = Array(SFX_POOL_SIZE)
    .fill()
    .map(() => new Audio("enemy_hurt.mp3"));
const shootSFX = Array(SFX_POOL_SIZE)
    .fill()
    .map(() => new Audio("shoot.mp3"));
        ...
        if (enemy.type === "basic") {
            ...
            if (enemy.isHovering) {
                ...
                if (distToTarget > enemy.speed) {
                    enemy.x += (dxToTarget / distToTarget) * enemy.speed;
                    enemy.y += (dyToTarget / distToTarget) * enemy.speed;
                } else {
